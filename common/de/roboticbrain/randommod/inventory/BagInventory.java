...
public class BagInventory implements IInventory {
    ...
    private final int INVENTORY_SIZE = 4*13;

    public BagInventory(ItemStack stack) {
        if(stack.getItem() != ModItems.backPack) return;
    ...
    private static final String NBT_BAG_INVENTORY = "alchemicalBagInventory";

    private boolean readFromNBT(NBTTagCompound tag) {
        if(tag == null || !tag.hasKey(NBT_BAG_INVENTORY)) return false;
        NBTTagList tagList = tag.getTagList(NBT_BAG_INVENTORY);
    ...
    public void writeBagContents(ItemStack item) {
        ...
        tag.setTag(NBT_BAG_INVENTORY, list);
    }
}
